const Work = {
    ...
    '<div>' +
        ...
        '<p class="text-decoration-none m-0 text-white" style="font-size: .8rem; line-height: normal; opacity: 0.7">{{ artist.name_de }}</p>' +
        '</div>' +
        '</router-link>\n' +
        '</div>' +
        '</div>' +
        ...
        fetchData() {
            ...
            fetch(page_url)
                ...
                .then(res => {
                    ...
                    this.name = res.work.name_de;
                    this.description = res.work.description_de;
                    this.field_artists = res.works.field_artists_de;
                    this.field_software = res.works.field_software_de;
                })
        }
    }
